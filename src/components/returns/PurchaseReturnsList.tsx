...
import { mockPurchaseReturns } from "@/models/returns";
...
export function PurchaseReturnsList({ dateRange, supplierId, searchQuery }: PurchaseReturnsListProps) {
  const [openDeleteDialog, setOpenDeleteDialog] = React.useState(false);
  const [returnToDelete, setReturnToDelete] = React.useState<string | null>(null);
  const navigate = useNavigate();
  const { toast } = useToast();

  // Filter purchase returns based on date range, supplier, and search query
  const filteredReturns = mockPurchaseReturns.filter(returnItem => {
    ...
    // Filter by supplier
    const isMatchingSupplier = !supplierId || returnItem.supplierId === supplierId;
    ...
    return isInDateRange && isMatchingSupplier && matchesSearch;
  });
  ...
  const handleView = (returnId: string) => {
    navigate(`/returns/purchase/${returnId}`);
  };
                ...
                {filteredReturns.length === 0 ? (
                  ...
                  filteredReturns.map((returnItem) => (
                    <TableRow key={returnItem.id}>
                      ...
                      <TableCell>{returnItem.purchaseOrderId}</TableCell>
